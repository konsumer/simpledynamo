...
test('put', async () => {
  ...
  // put a bunch of records to trigger looping query
  for (let i = 0; i < 100; i++) {
    await db.put({ name: "Kickin' Kicks", type: 'pants' })
  }
})
...
test('scan', async () => {
  const records = await db.scan({ all: true })
  assert(records)
  assert.equal(records.length, 101, 'Wrong return-length')
})
